...
static cp::CSV csv("test.csv");

class StereoMatch
{
public:
	StereoMatch(int blockSize, int minDisp, int disparityRange);
	virtual ~StereoMatch();
	...
	void gui(cv::Mat& leftim, cv::Mat& rightim, cv::Mat& dest, cp::StereoEval& eval);
	void gui(cv::Mat& leftim, cv::Mat& rightim, cv::Mat& dest, cp::StereoEval& eval, cv::Mat& dmapC, const int offset_pointcloud = 0);
...
protected:
	...
	cp::CrossBasedLocalFilter clf;//filtering function is thread safe.
	cp::GuidedImageFilter* gif;
